<?php
/**
 * Unified Seller Login Page
 * Used by both YFClaim and Classifieds modules
 */

// Check if we're in the refactor subdirectory or main directory
if (file_exists(__DIR__ . '/../vendor/autoload.php')) {
    require_once __DIR__ . '/../vendor/autoload.php';
} elseif (file_exists(dirname(dirname(__DIR__)) . '/config/database.php')) {
    // We're in a subdirectory, go up to find config
    require_once dirname(dirname(__DIR__)) . '/config/database.php';
} else {
    // Fallback to just database config
    require_once dirname(dirname(__DIR__)) . '/config/database.php';
}

use YFEvents\Application\Services\YFAuthService;

$authService = new YFAuthService($pdo);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    $result = $authService->authenticateSeller($email, $password);
    
    if ($result['success']) {
        // Redirect to return URL or dashboard
        $returnUrl = $_GET['return'] ?? '/seller/dashboard';
